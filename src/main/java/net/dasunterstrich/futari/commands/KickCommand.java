...
public class KickCommand extends BotCommand {
    ...
    public CommandData getModalCommandData(Command.Type type) {
        return Commands.context(type, "kick_modal")
    ...
    public void onTextCommand(MessageReceivedEvent event) {
        ...
        punisher.kick(event.getGuild(), targetMember, event.getMember(), reason, "", EvidenceMessage.empty()).handleAsync((communicationResponse, throwable) -> {
            if (throwable != null) {
                event.getChannel().sendMessageEmbeds(EmbedUtils.error("Could not kick " + targetMember.getUser().getAsTag() + "!\n\n**Reason**: " + throwable.getMessage())).queue();
                return null;
            }

            var text = "**Reason**: " + reason + (communicationResponse.isFailure() ? "\n\n**Warning: Could not contact user**" : "");
            event.getChannel().sendMessageEmbeds(EmbedUtils.success(targetMember.getUser().getAsTag() + " kicked", text)).queue();
            return null;
        });
    }
    ...
    public void onSlashCommand(SlashCommandInteractionEvent event) {
        ...
        punisher.kick(event.getGuild(), targetMember, event.getMember(), reason, comments, evidence).handleAsync((communicationResponse, throwable) -> {
            if (throwable != null) {
                event.getHook().editOriginalEmbeds(EmbedUtils.error("Could not kick " + targetMember.getUser().getAsTag() + "!\n\n**Reason**: " + throwable.getMessage())).queue();
                return null;
            }

            var text = "**Reason**: " + reason + (communicationResponse.isFailure() ? "\n\n**Warning: Could not contact user**" : "");
            event.getHook().editOriginalEmbeds(EmbedUtils.success(targetMember.getUser().getAsTag() + " kicked", text)).queue();
            return null;
        });
    }
    ...
    public void onModalInteraction(ModalInteractionEvent event) {
        ...
        event.getGuild().retrieveMemberById(event.getModalId().split(":")[1]).queue(targetMember -> {
            ...
            punisher.kick(event.getGuild(), targetMember, event.getMember(), reason, comments, evidenceMessage).handleAsync((communicationResponse, throwable) -> {
                if (throwable != null) {
                    event.getHook().editOriginalEmbeds(EmbedUtils.error("Could not kick " + targetMember.getUser().getAsTag() + "!\n\n**Reason**: " + throwable.getMessage())).queue();
                    return null;
                }
